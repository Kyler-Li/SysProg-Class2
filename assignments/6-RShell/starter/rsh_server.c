...
int boot_server(char *ifaces, int port){
    ...
    if (svr_socket == -1) {
        perror("The [Server] socket() failed");
        exit(EXIT_FAILURE);
    }
    ...
    ret = inet_pton(AF_INET, ifaces, &addr.sin_addr);
    if (ret <= 0) {
        perror("[Server] Invalid bind address");
        close(svr_socket);
    ...
    if (ret == -1) {
        perror("The [Server] bind() failed");
        close(svr_socket);
        exit(EXIT_FAILURE);
    }

    // Bind socket
    // if (bind(svr_socket, (struct sockaddr *) &addr, sizeof(struct sockaddr_in)) == -1) {
    //     perror("Bind failed");
    //     close(svr_socket);
    //     return ERR_RDSH_COMMUNICATION;
    // }
    ...
    if (listen(svr_socket, 5) == -1) {
        perror("The [Server] listen() failed");
        close(svr_socket);
        exit(EXIT_FAILURE);
        // return ERR_RDSH_COMMUNICATION;
    }

    printf("[Server] Listening on %s:%d", ifaces, port);
...
int process_cli_requests(int svr_socket){
    ...
    while (1) {
        ...
        client_fd = accept(svr_socket, (struct sockaddr *)&client_addr, &addr_len);
        if (client_fd == -1) {
        ...
        exec_client_requests(client_fd);
        close(client_fd);
...
int build_cmd_list(char *cmd_line, command_list_t *clist) {
    ...
    char *token = strtok(cmd_line, PIPE_STRING); // Split by `|`
...
int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        // do the recv();
        recv_size = recv(cli_socket, buffer, RDSH_COMM_BUFF_SZ - 1, 0);
        if (recv_size <= 0) {
            free(cmd_buff);
            if (recv_size == 0) {
                printf("[Server] Client disconnected.\n");
        ...
        if (strcmp(buffer, "exit") == 0) {
            perror("[Server] Client requested exit.");
            free(cmd_buff);
            return OK;
        }
        
        if (strcmp(buffer, "stop-server") == 0) {
            printf("[Server] Client requested server shutdown.\n");
            free(cmd_buff);
            return OK_EXIT;
        }

        if (build_cmd_list(buffer, &clist) != OK) {
            perror("[Server] Command parsing failed");
            free(cmd_buff);
            return ERR_RDSH_CMD_EXEC;
        }
        // check the number of bytes of recv
        // build a cmd_list from previous assignment
        // build rshell execute pipeline
        // takes giant string from user
...
int send_message_string(int cli_socket, char *buff){
    ssize_t bytes_sent = send(cli_socket, buff, strlen(buff), 0);
    if (bytes_sent == -1) {
